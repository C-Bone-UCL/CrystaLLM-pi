...
"""
Hyperparameter search script for PKV conditional crystal structure generation fine-tuning.
Optimizes PKV-specific parameters using Optuna for improved conditional generation performance.
"""
...
def optuna_hp_space(trial):
    """Define hyperparameter search space focused on PKV conditioning parameters for small dataset."""
    hp = {}
    
    # PKV Architecture - More conservative for small dataset
    ...
    hp["warmup_steps"] = trial.suggest_int("warmup_steps", 50, 500, step=50)
...
def main():
    """Main hyperparameter search function for PKV fine-tuning optimization."""
    ...
    os.environ["WANDB_PROJECT"] = "hpsearch_PKV_cllmv2"
    ...
    if torch.cuda.device_count() > 1:
        ...
        print(f"Multiple GPUs detected. Using only GPU 0 for hyperparameter search")
    ...
    # Load data for PKV conditioning
    if args.activate_conditionality == "PKV":
        print("\nPKV CONDITIONALITY ACTIVATED")
        
        # Handle condition_columns format (string or list)
        condition_columns = args.condition_columns
        if isinstance(condition_columns, str):
            import ast
            try:
                condition_columns = ast.literal_eval(condition_columns)
            except:
    ...
    best_run = trainer.hyperparameter_search(
        ...
        study_name="PKV_slme",
        storage="sqlite:///optuna_hpsearch_PKV_slme.db",
        load_if_exists=True,
        n_jobs=1,
